...
inline FXbool IconItem::isOdd(FXint i) const
{
	return (FXbool)(i & 0x1);
}

// Draw big icon
void IconItem::drawBigIcon(const IconList* list,FXDC& dc,FXint x,FXint y,FXint w,FXint h) const
{
...
// Draw mini icon
void IconItem::drawMiniIcon(const IconList* list,FXDC& dc,FXint x,FXint y,FXint w,FXint h) const {
	register FXFont *font=list->getFont();
...
void IconItem::drawDetails(IconList* list,FXDC& dc,FXint x,FXint y,FXint,FXint h) const
{
	register FXHeader *header=list->getHeader();
	register FXFont *font=list->getFont();
	register FXint iw=0,ih=0,tw=0,th=0,yt,beg,end,hi,drw,space,used,dw,xx,dx=0;
	...
	for (hi=0; hi<(FXint)list->getSortHeader(); hi++)
...
FXint IconItem::hitItem(const IconList* list,FXint rx,FXint ry,FXint rw,FXint rh) const
{
...
// Get item width
FXint IconItem::getWidth(const IconList* list) const
{
	register FXuint options=list->getListStyle();
	register FXFont *font=list->getFont();
...
// Get item height
FXint IconItem::getHeight(const IconList* list) const
{
	register FXuint options=list->getListStyle();
...
FXDEFMAP(IconList) IconListMap[]=
{
	...
	// Desactivated this one because of a conflict with KeybindingsBox
	// I'll check if it is necessary
	//FXMAPFUNC(SEL_COMMAND,0,IconList::onCommand),
...
long IconList::onConfigure(FXObject*,FXSelector,void*)
{
	...
	FXint width=(this->getShell())->getWidth();
	
	// Initialize the relative sizes, skipping the first call
	if (count==1)
	{
		// Initialize the relative header sizes
		for (FXint hi=0; hi<getNumHeaders(); hi++)
			headerpct[hi]=(FXdouble)getHeaderSize(hi)/(FXdouble)width;
	}
	...
	if (relative_resize && count>=1)
	{
		...
		{
			headerpct[8]=(FXdouble)getHeaderSize(8)/(FXdouble)width;
			initheaderpct=FALSE;
		}
		if (initheaderpct && getNumHeaders()==10)
		{
			headerpct[8]=(FXdouble)getHeaderSize(8)/(FXdouble)width;
			headerpct[9]=(FXdouble)getHeaderSize(9)/(FXdouble)width;
			initheaderpct=FALSE;
		}
		FXint newhsize;
		for (FXint hi=0; hi<getNumHeaders(); hi++)
		{
			newhsize=(FXint)round(headerpct[hi]*width);
...
// Move content
void IconList::moveContents(FXint x,FXint y)
{
	FXint dx=x-pos_x;
	FXint dy=y-pos_y;
	FXint top=0;
...
FXint IconList::getContentWidth()
{
...
FXint IconList::getContentHeight()
{
	if (flags&FLAG_RECALC)
		recompute();
...
void IconList::resize(FXint w,FXint h)
{
	FXint nr=nrows;
	FXint nc=ncols;
	if (w!=width || h!=height)
...
// we don't want to re-measure the items, but the content
// size has changed because the number of rows/columns has...
void IconList::position(FXint x,FXint y,FXint w,FXint h)
{
	FXint nr=nrows;
	FXint nc=ncols;
	if (w!=width || h!=height)
...
void IconList::setHeaders(const FXchar** strings,FXint size)
{
	header->clearItems();
...
void IconList::setHeaders(const FXString& strings,FXint size)
{
	header->clearItems();
	header->fillItems(strings,NULL,size);
}
...
void IconList::appendHeader(const FXString& text,FXIcon *icon,FXint size)
{
...
void IconList::removeHeader(FXint index)
{
...
void IconList::setHeaderText(FXint index,const FXString& text)
{
	if (index<0 || header->getNumItems()<=index)
	{
...
// Get header caption
FXString IconList::getHeaderText(FXint index) const
{
	if (index<0 || header->getNumItems()<=index)
	{
...
// Change header icon
void IconList::setHeaderIcon(FXint index,FXIcon *icon)
{
	if (index<0 || header->getNumItems()<=index)
	{
...
// Get header icon
FXIcon* IconList::getHeaderIcon(FXint index) const
{
	if (index<0 || header->getNumItems()<=index)
	{
...
// Change header size
void IconList::setHeaderSize(FXint index,FXint size)
{
	if (index<0 || header->getNumItems()<=index)
	{
...
// Get header size
FXint IconList::getHeaderSize(FXint index) const
{
	if (index<0 || header->getNumItems()<=index)
	{
...
FXint IconList::getNumHeaders() const
{
...
// Change item text
void IconList::setItemText(FXint index,const FXString& text)
{
...
// Get item text
FXString IconList::getItemText(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
// Set item icon
void IconList::setItemBigIcon(FXint index,FXIcon* icon,FXbool owned)
{
	if (index<0 || items.no()<=index)
	{
...
// Get item icon
FXIcon* IconList::getItemBigIcon(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
// Set item icon
void IconList::setItemMiniIcon(FXint index,FXIcon* icon,FXbool owned)
{
	if (index<0 || items.no()<=index)
	{
...
// Get item icon
FXIcon* IconList::getItemMiniIcon(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
// Set item data
void IconList::setItemData(FXint index,void* ptr)
{
	if (index<0 || items.no()<=index)
	{
...
// Get item data
void* IconList::getItemData(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
FXbool IconList::isItemCurrent(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
FXbool IconList::isItemEnabled(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
FXbool IconList::isItemVisible(FXint index) const
{
...
void IconList::makeItemVisible(FXint index)
{
...
//FXint IconList::getItemAt(FXint x,FXint y) const
FXint IconList::getItemAt(FXint x,FXint y)
{
...
// Get item by name
FXint IconList::findItem(const FXString& text,FXint start,FXuint flgs) const
{
...
// Get item by data
FXint IconList::findItemByData(const void *ptr,FXint start,FXuint flgs) const
{
...
FXint IconList::hitItem(FXint index,FXint x,FXint y,FXint ww,FXint hh) const
{
...
// Enable one item
FXbool IconList::enableItem(FXint index)
{
	if (index<0 || items.no()<=index)
	{
...
// Disable one item
FXbool IconList::disableItem(FXint index)
{
	if (index<0 || items.no()<=index)
	{
...
// Select one item
FXbool IconList::selectItem(FXint index,FXbool notify)
{
	if (index<0 || items.no()<=index)
	{
...
// Deselect one item
FXbool IconList::deselectItem(FXint index,FXbool notify)
{
	if (index<0 || items.no()<=index)
	{
...
// Toggle one item
FXbool IconList::toggleItem(FXint index,FXbool notify)
{
	if (index<0 || items.no()<=index)
	{
...
FXbool IconList::selectInRectangle(FXint x,FXint y,FXint w,FXint h,FXbool notify)
{
	register FXint r,c,index;
...
// Extend selection
FXbool IconList::extendSelection(FXint index,FXbool notify)
{
...
void IconList::lassoChanged(FXint ox,FXint oy,FXint ow,FXint oh,FXint nx,FXint ny,FXint nw,FXint nh,FXbool notify)
{
	register FXint r,c;
...
long IconList::onPaint(FXObject*,FXSelector,void* ptr)
{
	...
	// Icon mode
		...
		if (this->listType == IconList_ListType_LargeIcons) {
			for (r=rlo; r<=rhi; r++)
			{
				...
				for (c=clo; c<=chi; c++)
				{
					...
					if (index<items.no())
					{
						items[index]->drawBigIcon(this,dc,x,y,itemSpace,itemHeight);
					}
				}
			}
		} else { // Mini icons
			for (r=rlo; r<=rhi; r++) {
				...
				for (c=clo; c<=chi; c++) {
					...
					if (index<items.no()) {
						items[index]->drawMiniIcon(this,dc,x,y,itemSpace,itemHeight);
					}
				}
			}
		}
		...
		for (index=rlo; index<=rhi; index++,y+=itemHeight)
		{
			...
			items[index]->drawDetails(this,dc,pos_x,y,width,itemHeight);
		}
...
void IconList::drawLasso(FXint x0,FXint y0,FXint x1,FXint y1)
{
	FXDCWindow dc(this);
...
FXint IconList::compareSection(const FXchar *p,const FXchar* q,FXint s)
{
...
FXint IconList::compareSectionCase(const FXchar *p,const FXchar* q,FXint s)
{
	register FXint c1,c2,x;
	for (x=s; x && *p; x-=(*p++=='\t'));
	for (x=s; x && *q; x-=(*q++=='\t'));
...
FXint IconList::ascending(const IconItem* a,const IconItem* b)
{
...
FXint IconList::descending(const IconItem* a,const IconItem* b)
{
...
FXint IconList::ascendingCase(const IconItem* a,const IconItem* b)
{
...
FXint IconList::descendingCase(const IconItem* a,const IconItem* b)
{
...
// Set current item
void IconList::setCurrentItem(FXint index,FXbool notify)
{
...
// Set anchor item
void IconList::setAnchorItem(FXint index)
{
	if (index<-1 || items.no()<=index)
	{
...
long IconList::onKeyPress(FXObject*,FXSelector,void* ptr)
{
	...
	case KEY_KP_Page_Up:
		...
		index-=(FXint)(verticalScrollBar()->getPage()/verticalScrollBar()->getLine());
		goto hop;
		return 1;
	case KEY_Page_Down:
...
// Retrieve item
IconItem *IconList::getItem(FXint index) const
{
	if (index<0 || items.no()<=index)
	{
...
FXint IconList::setItem(FXint index,IconItem* item,FXbool notify)
{
...
// Replace item with another
FXint IconList::setItem(FXint index,const FXString& text,FXIcon *big,FXIcon* mini,void* ptr,FXbool notify)
{
...
// Insert item
FXint IconList::insertItem(FXint index,IconItem* item,FXbool notify)
{
...
// Insert item
FXint IconList::insertItem(FXint index,const FXString& text,FXIcon *big,FXIcon* mini,void* ptr,FXbool notify)
{
...
// Append item
FXint IconList::appendItem(IconItem* item,FXbool notify)
{
...
// Append item
FXint IconList::appendItem(const FXString& text,FXIcon *big,FXIcon* mini,void* ptr,FXbool notify)
{
...
// Prepend item
FXint IconList::prependItem(IconItem* item,FXbool notify)
{
...
// Prepend item
FXint IconList::prependItem(const FXString& text,FXIcon *big,FXIcon* mini,void* ptr,FXbool notify)
{
...
FXint IconList::fillItems(const FXchar** strings,FXIcon *big,FXIcon* mini,void* ptr,FXbool notify)
{
	register FXint n=0;
	if (strings)
	{
...
FXint IconList::fillItems(const FXString& strings,FXIcon *big,FXIcon* mini,void* ptr,FXbool notify)
{
	register FXint n=0;
	FXString text;
...
FXint IconList::moveItem(FXint newindex,FXint oldindex,FXbool notify)
{
	register FXint old=current;
...
IconItem* IconList::extractItem(FXint index,FXbool notify)
{
...
void IconList::removeItem(FXint index,FXbool notify)
{
	register FXint old=current;
