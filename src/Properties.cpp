...
PropertiesBox::PropertiesBox(FXWindow *win,FXString file,FXString path): DialogBox(win,_("Properties"), DECOR_TITLE|DECOR_BORDER|DECOR_STRETCHABLE)
{
    ...
    FXchar mnttype[64], used[64], avail[64], pctr[64], size[64];
    FXchar buf[MAXPATHLEN+1];
	FXString hsize;
	FILE *p;
    ...
    {		
		...
		if(isMountpoint)
		{
			...
			snprintf(buf,sizeof(buf)-1,"df -P --block-size=1 '%s'",filepath.text());
			p=popen(buf,"r");
	...
	if (isInTrash)
	{
		...
		if (deldate!=0)
			delstr=FXSystem::time("%c",deldate);
...
PropertiesBox::PropertiesBox(FXWindow *win,FXString *file,int n,FXString path): DialogBox(win,_("Properties"), DECOR_TITLE|DECOR_BORDER|DECOR_STRETCHABLE)
{
    ...
    FXString type, extension, fileassoc;
    FXchar buf[MAXPATHLEN+1];
	FXint i, isDirectory, nbseldirs=0, nbselfiles=0, dotdot=0;
	...
	if (open->getText()!=open_prev || descr->getText()!=descr_prev || bigic->getText()!=bigic_prev || miniic->getText()!=miniic_prev) {
		...
		if (ext->getText()!="")
		{
			...
			if(oldfileassoc == "" || fileassoc.section('=',1) != oldfileassoc)
			{
				...
				str=new FXchar*[2];
				str[0]=new FXchar[strlen(ext->getText().text())+1];
				str[1]=new FXchar[strlen(command.text())+1];
...
long PropertiesBox::onUpdSizeAndPerm(FXObject* o,FXSelector s,void* p)
{
	...
	if (recsize)
	{
		...
		else
		{
			...
			FXuint totalnbfiles=0, totalnbsubfolders=0;
			struct stat info;
			FXint dotdot=0;
