...
FXString getKeybinding(FXEvent* event)
{
	...
	FXint mods=event->state;
	FXint code=event->code;
...
FXint mkpath(const FXchar* s, mode_t mode)
{
	FXchar *q, *r = NULL, *path = NULL, *up = NULL;
	FXint rv;
...
size_t strlcpy(FXchar* dst, const char* src, size_t sz)
{
	const FXchar *start = src;
...
// The destination string is always null terminated
size_t strlcat(FXchar* dst, const FXchar* src, size_t sz)
{
   FXchar *start = dst;
...
unsigned long long dirsize(const FXchar* path)
{
    DIR *dp;
	struct dirent *dirp;
	struct stat statbuf;
...
unsigned long long pathsize(FXchar* path, FXuint* nbfiles, FXuint* nbsubdirs)
{
	struct stat statbuf;
	struct dirent *dirp;
    FXchar *ptr;
...
FXint hasSubDirs(const FXString directory)
{
	FXint ret=-1;
	DIR* dir;
	struct dirent *entry;
...
{	
	...
	{
		FXint ret=access(file.text(),R_OK|X_OK);
		if (ret==0)
			return TRUE;
		else
			return FALSE;
...
{	
	...
	if (!file.empty() && (statrep(file.text(),&info)==0))
	{
		FXint ret=access(file.text(),R_OK);
		if (ret==0)
			return TRUE;
		else
			return FALSE;
...
{	
	...
	if (!file.empty() && (statrep(file.text(),&info)==0))
	{
		FXint ret=access(file.text(),W_OK);
		if (ret==0)
			return TRUE;
		else
			return FALSE;
...
FXint setWaitCursor(FXApp* app, FXuint type)
{
	static FXint waitcount=0;
