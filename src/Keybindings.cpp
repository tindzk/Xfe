        ...
        DialogBox(win,_("Key Bindings"), DECOR_TITLE|DECOR_BORDER|DECOR_RESIZE,0,0,800,600)
{
    ...
    FXVerticalFrame *frame1=new FXVerticalFrame(tabbook,LAYOUT_SIDE_TOP|FRAME_THICK|FRAME_RAISED|LAYOUT_FILL_X|LAYOUT_FILL_Y);
    ...
    FXVerticalFrame *frame2=new FXVerticalFrame(tabbook,LAYOUT_SIDE_TOP|FRAME_THICK|FRAME_RAISED|LAYOUT_FILL_X|LAYOUT_FILL_Y);
    ...
    FXVerticalFrame *frame3=new FXVerticalFrame(tabbook,LAYOUT_SIDE_TOP|FRAME_THICK|FRAME_RAISED|LAYOUT_FILL_X|LAYOUT_FILL_Y);
    ...
    FXVerticalFrame *frame4=new FXVerticalFrame(tabbook,LAYOUT_SIDE_TOP|FRAME_THICK|FRAME_RAISED|LAYOUT_FILL_X|LAYOUT_FILL_Y);
...
long KeybindingsBox::onCmdAccept(FXObject* o,FXSelector s,void* p)
{
	...
	if (changed)
	{
		...
		for (FXint i=glbBindingsDict->first(); i<glbBindingsDict->size(); i=glbBindingsDict->next(i))
		{
		...
		for (FXint i=xfeBindingsDict->first(); i<xfeBindingsDict->size(); i=xfeBindingsDict->next(i))
		{
		...
		for (FXint i=xfiBindingsDict->first(); i<xfiBindingsDict->size(); i=xfiBindingsDict->next(i))
		{
		...
		for (FXint i=xfwBindingsDict->first(); i<xfwBindingsDict->size(); i=xfwBindingsDict->next(i))
		{
...
long KeybindingsBox::onCmdCancel(FXObject* o,FXSelector s,void* p)
{
    ...
    for (FXint i=glbBindingsDict_prev->first(); i<glbBindingsDict_prev->size(); i=glbBindingsDict_prev->next(i))
	{
		regkey=glbBindingsDict_prev->key(i);
		data=glbBindingsDict_prev->data(i);
		glbBindingsDict->replace(regkey.text(),data.text());
	}
    for (FXint i=xfeBindingsDict_prev->first(); i<xfeBindingsDict_prev->size(); i=xfeBindingsDict_prev->next(i))
	{
		regkey=xfeBindingsDict_prev->key(i);
		data=xfeBindingsDict_prev->data(i);
		xfeBindingsDict->replace(regkey.text(),data.text());
	}
    for (FXint i=xfiBindingsDict_prev->first(); i<xfiBindingsDict_prev->size(); i=xfiBindingsDict_prev->next(i))
	{
		regkey=xfiBindingsDict_prev->key(i);
		data=xfiBindingsDict_prev->data(i);
		xfiBindingsDict->replace(regkey.text(),data.text());
	}
    for (FXint i=xfwBindingsDict_prev->first(); i<xfwBindingsDict_prev->size(); i=xfwBindingsDict_prev->next(i))
	{
...
FXint KeybindingsBox::compareSection(const FXchar *p,const FXchar* q,FXint s)
{
...
// Sort functions
FXint KeybindingsBox::ascendingActionName(const IconItem* a,const IconItem* b)
{
    return compareSection(a->getText().text(),b->getText().text(),0);
}


FXint KeybindingsBox::descendingActionName(const IconItem* a,const IconItem* b)
{
    return compareSection(b->getText().text(),a->getText().text(),0);
}

FXint KeybindingsBox::ascendingRegistryKey(const IconItem* a,const IconItem* b)
{
    return compareSection(a->getText().text(),b->getText().text(),1);
}


FXint KeybindingsBox::descendingRegistryKey(const IconItem* a,const IconItem* b)
{
    return compareSection(b->getText().text(),a->getText().text(),1);
}


FXint KeybindingsBox::ascendingKeybinding(const IconItem* a,const IconItem* b)
{
    return compareSection(a->getText().text(),b->getText().text(),2);
}


FXint KeybindingsBox::descendingKeybinding(const IconItem* a,const IconItem* b)
{
...
long KeybindingsBox::onCmdDefineGlbKeybindings(FXObject*,FXSelector,void*)
{
	...
	FXString itemtext="";
	FXint index=-1;
	for (FXint u=0; u< glbBindingsList->getNumItems (); u++)
	{
...
long KeybindingsBox::onCmdDefineXfeKeybindings(FXObject*,FXSelector,void*)
{
	...
	FXString itemtext="";
	FXint index=-1;
	for (FXint u=0; u< xfeBindingsList->getNumItems (); u++)
	{
...
long KeybindingsBox::onCmdDefineXfiKeybindings(FXObject*,FXSelector,void*)
{
	...
	FXString itemtext="";
	FXint index=-1;
	for (FXint u=0; u< xfiBindingsList->getNumItems (); u++)
	{
...
long KeybindingsBox::onCmdDefineXfwKeybindings(FXObject*,FXSelector,void*)
{
	...
	FXString itemtext="";
	FXint index=-1;
	for (FXint u=0; u< xfwBindingsList->getNumItems (); u++)
	{
