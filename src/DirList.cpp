...
fnd:
            *pn=item;
			...
			// Data passed to the item->data pointer and used to update the tooltip
			FXString strdata=item->label+"\t"+type+"\t"+mod+"\t"+usrid+"\t"+grpid+"\t"+atts+"\t"+del+"\t"+pathname;
			FXMALLOC(&item->data,FXchar,strdata.length()+1);
			memcpy(item->data,strdata.text(),strdata.length()+1);	
