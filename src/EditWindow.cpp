...
Preferences::Preferences(EditWindow *owner):DialogBox(owner,_("XFileWrite Preferences"),DECOR_TITLE|DECOR_BORDER)
{
    ...
    FXint wrapcols=getApp()->reg().readIntEntry("OPTIONS","wrapcols",80);
    ...
    FXint tabcols=getApp()->reg().readIntEntry("OPTIONS","tabcols",80);
    ...
    FXint stripreturn=getApp()->reg().readIntEntry("OPTIONS","stripreturn",FALSE);
...
FXDEFMAP(EditWindow) EditWindowMap[]=
{
	...
	FXMAPFUNC(SEL_SIGNAL,EditWindow::ID_HARVEST,EditWindow::onSigHarvest),
...
// Make some windows
EditWindow::EditWindow(XFileWrite* a,const FXString& file):FXMainWindow(a,"XFileWrite",NULL,NULL,DECOR_ALL),mrufiles(a)
{
    FXHotKey hotkey;
	FXString key;
    ...
    // Make editor window
    FXHorizontalFrame *textbox=new FXHorizontalFrame(this,FRAME_NONE|LAYOUT_FILL_X|LAYOUT_FILL_Y,0,0,0,0, 0,0,0,0);
    ...
    new FXButton(toolbar,TAB+_("Goto line")+PARS(key)+TAB+_("Goto line number."),gotolineicon,this,EditWindow::ID_GOTO_LINE,ICON_ABOVE_TEXT|BUTTON_TOOLBAR|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);
    ...
    new FXButton(toolbar,TAB+_("Search")+PARS(key)+TAB+_("Search text.")+PARS(key),searchicon,this,EditWindow::ID_SEARCH,ICON_ABOVE_TEXT|BUTTON_TOOLBAR|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);

	key=getApp()->reg().readStringEntry("KEYBINDINGS","search_prev","Shift-Ctrl-G");
    new FXButton(toolbar,TAB+_("Search selection backward")+PARS(key)+TAB+_("Search backward for selected text.")+PARS(key),searchprevicon,this,EditWindow::ID_SEARCH_BACK_SEL,ICON_ABOVE_TEXT|BUTTON_TOOLBAR|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);

	key=getApp()->reg().readStringEntry("KEYBINDINGS","search_next","Ctrl-G");
    new FXButton(toolbar,TAB+_("Search selection forward")+PARS(key)+TAB+_("Search forward for selected text.")+PARS(key),searchnexticon,this,EditWindow::ID_SEARCH_FORW_SEL,ICON_ABOVE_TEXT|BUTTON_TOOLBAR|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);
    ...
    mc=new FXMenuCommand(editmenu,text,gotolineicon,this,EditWindow::ID_GOTO_LINE);
	hotkey=_parseAccel(key);
	getAccelTable()->addAccel(hotkey,mc,FXSEL(SEL_COMMAND,FXMenuCommand::ID_ACCEL));
    ...
    mc=new FXMenuCommand(searchmenu,text,searchicon,this,EditWindow::ID_SEARCH);
	hotkey=_parseAccel(key);
	getAccelTable()->addAccel(hotkey,mc,FXSEL(SEL_COMMAND,FXMenuCommand::ID_ACCEL));
    ...
    mc=new FXMenuCommand(searchmenu,text,replaceicon,this,EditWindow::ID_REPLACE);
	hotkey=_parseAccel(key);
	getAccelTable()->addAccel(hotkey,mc,FXSEL(SEL_COMMAND,FXMenuCommand::ID_ACCEL));

	key=getApp()->reg().readStringEntry("KEYBINDINGS","search_prev","Shift-Ctrl-G");
	text=_("Search sel. &backward")+TABS(key)+_("Search backward for selected text.")+PARS(key);
    mc=new FXMenuCommand(searchmenu,text,searchprevicon,this,EditWindow::ID_SEARCH_BACK_SEL);
	hotkey=_parseAccel(key);
	getAccelTable()->addAccel(hotkey,mc,FXSEL(SEL_COMMAND,FXMenuCommand::ID_ACCEL));
    
	key=getApp()->reg().readStringEntry("KEYBINDINGS","search_next","Ctrl-G");
	text=_("Search sel. &forward")+TABS(key)+_("Search forward for selected text.")+PARS(key);
	mc=new FXMenuCommand(searchmenu,text,searchnexticon,this,EditWindow::ID_SEARCH_FORW_SEL);
	hotkey=_parseAccel(key);
	getAccelTable()->addAccel(hotkey,mc,FXSEL(SEL_COMMAND,FXMenuCommand::ID_ACCEL));
...
void EditWindow::loadConfig()
{
    ...
    FXint wrapping,wrapcols,tabcols;
...
// Harvest the zombies
long EditWindow::onSigHarvest(FXObject*,FXSelector,void*)
{
