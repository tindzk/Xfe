...
{	
	...
	// File list width
	//FXint frameWidth = filelist->getWidth();
    ...
    visitedPath = path;
    //FXint textWidth = normalFont->getTextWidth(path.text(), path.length());
...
long PathLinker::pathButtonPressed(FXObject * obj ,FXSelector sel ,void* ptr)
{
	...
	{
		FXint rpos = 0;
		rpos = visitedPath.rfind((FXchar)PATHSEPSTRING[0], 0x7FFFFFFF, nbActiveButtons - (endId - ID_START_LINK + 1));
	...
	if (dirlist)
		dirlist->setDirectory(FXPath::directory(filePath),TRUE);
...
{	
	...
	{			
		...
		if (currentButton==0)
			...
			currentlink = visitedPath.before((FXchar)PATHSEPSTRING[0],currentButton+1);
		...
		while (path != "")
		{
			...
			// Next path to test
			path=visitedPath.rbefore((FXchar)PATHSEPSTRING[0],n);
			n++;
		}
