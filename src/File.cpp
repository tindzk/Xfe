...
// Construct object
File::File(FXWindow *owner, FXString title, const FXuint operation):DialogBox(owner,title,DECOR_TITLE|DECOR_BORDER|DECOR_STRETCHABLE)
{
    // Progress window
...
// Read bytes
long File::fullread(FXint fd, FXuchar* ptr,long len)
{
    long nread;
#ifdef EINTR
    do
...
// Write bytes
long File::fullwrite(FXint fd, const FXuchar* ptr,long len)
{
...
FXuint File::getOverwriteAnswer(FXString srcpath, FXString tgtpath)
{
	...
	{
		statsrc=TRUE;
		srcmtime=FXSystem::time("%c",linfo.st_mtime);
		...
		if (S_ISDIR(linfo.st_mode)) // Folder
		{
			...
			snprintf(buf,sizeof(buf),"%llu",(unsigned long long)linfo.st_size);					
		srcsize=::hSize(buf);
	}			
	if (lstatrep(tgtpath.text(),&linfo)==0)
	{
		stattgt=TRUE;
		tgtmtime=FXSystem::time("%c",linfo.st_mtime);
		FXchar buf[64];
		if (S_ISDIR(linfo.st_mode)) // Folder
		{
			unsigned long long dirsize=0;
			...
			if((dst=::open(destfile.text(),O_WRONLY|O_CREAT|O_TRUNC,info.st_mode))>=0)
            {
                while(1)
                {
                    ...
                    if(nread<0)
					{
						...
						FXuint answer;
						if (errcode)
							answer=MessageBox::error(this,BOX_OK_CANCEL,_("Error"),_("Can't copy file %s: %s"),target.text(),strerror(errcode));
						else
							answer=MessageBox::error(this,BOX_OK_CANCEL,_("Error"),_("Can't copy file %s"),target.text());
				...
				if (preserve_date)
				{
					...
					errno=0;
					FXint rc=utime(destfile.text(),&timbuf);
					FXint errcode=errno;
					if (rc)
					{
						forceTimeout();
						FXuint answer;
						if (errcode)
							answer=MessageBox::error(this,BOX_OK_CANCEL,_("Error"),_("Can't preserve date when copying file %s : %s"),target.text(),strerror(errcode));
						else
							answer=MessageBox::error(this,BOX_OK,_("Error"),_("Can't preserve date when copying file %s"),target.text());
	...
	if (preserve_date)
	{
		if(lstatrep(source.text(),&linfo)==0)
		{
			...
			if (rc)
			{
				...
				FXuint answer;
				if (errcode)
					answer=MessageBox::error(this,BOX_OK_CANCEL,_("Error"),_("Can't preserve date when copying folder %s : %s"),target.text(),strerror(errcode));
				else
					answer=MessageBox::error(this,BOX_OK_CANCEL,_("Error"),_("Can't preserve date when copying folder %s"),target.text());
				restartTimeout();
				if(answer == BOX_CLICKED_CANCEL)
				{
					closedir(dirp);
...
FXbool File::remove(const FXString& file)
{
    ...
    if(lstatrep(file.text(),&linfo)==0)
    {
		...
		if(S_ISDIR(linfo.st_mode))
        {
			...
			{
				forceTimeout();
				MessageBox::error(this,BOX_OK,_("Error"),_("Can't delete folder %s"), file.text());
				restartTimeout();			
				return FALSE;
			}
			else
        ...
        else
        {
			...
			{
				forceTimeout();
				MessageBox::error(this,BOX_OK,_("Error"),_("Can't delete file %s"), file.text());	
				restartTimeout();		
				return FALSE;
			}
			else
...
FXint File::chmod(FXchar* path, FXchar* file, mode_t mode, FXbool rec, const FXbool dironly, const FXbool fileonly)
{			
    struct stat linfo;
...
// We don't process symbolic links (since their permissions cannot be changed)
FXint File::rchmod(FXchar* path, FXchar* file, mode_t mode, const FXbool dironly, const FXbool fileonly)
{
    struct stat linfo;
	...
	{	
        if (entry->d_name[0] != '.' || (entry->d_name[1] != '\0'
                                        && (entry->d_name[1] != '.' ||
                                            entry->d_name[2] != '\0')))
        {
            FXint pl1 = pl, l = strlen(entry->d_name);
            FXchar* path1 = (FXchar*)alloca(pl1+l+2);
...
// It can be different from the initial path, if recursive chmod is used
// (Used to fill an error message, if needed) 
FXint File::chown(FXchar* path, FXchar* file, uid_t uid, gid_t gid, const FXbool rec, const FXbool dironly, const FXbool fileonly)
{
    struct stat linfo;
...
// We don't follow symbolic links
FXint File::rchown(FXchar* path, FXchar* file, uid_t uid, gid_t gid, const FXbool dironly, const FXbool fileonly)
{
    struct stat linfo;
...
FXint File::extract(const FXString name, const FXString dir, const FXString cmd)
{
...
// Create an archive
FXint File::archive(const FXString name, const FXString cmd)
{
	// Target file already exists
